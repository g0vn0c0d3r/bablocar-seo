...
|---|---|---|
| Не делим | Метаданные страницы; Hero / первый экран; Почему выбирают Bablocar; Как получить деньги; Предложения партнеров; Документы и требования; Транспорт; Финальный CTA | Держим единый порядок, дизайн, условия, CTA и логику заявки. Title, H1 и подзаголовок подставляются под URL, но уникальность здесь не разгоняем. Description держим шаблонным. |
...
Почему так: первый экран и фиксированные блоки нужны для ясного предложения, доверия и конверсии. Если менять их случайно, страницы начнут противоречить друг другу по условиям и действию. Уникальность лучше создавать ниже по странице: через реальные сценарии, отзывы, FAQ, SEO-текст и ссылки на связанные страницы.
...
- Разные преимущества на похожих страницах могут создавать разные обещания.
...
## Блок 4. Предложения партнеров

### Скриншоты

| Городской хаб | Гео-интент | Гео-модификатор |
|---|---|---|
| <img src="./block_review_assets/offers-city.png" alt="Предложения партнеров городского хаба" width="320"> | <img src="./block_review_assets/offers-intent.png" alt="Предложения партнеров гео-интента" width="320"> | <img src="./block_review_assets/offers-modifier.png" alt="Предложения партнеров гео-модификатора" width="320"> |

**SEO-роль:** показывает, что Bablocar подбирает предложения, а не ведет к одному фиксированному займу. Блок не используем как главный источник SEO-уникальности.

### Проблемы

- Данные предложений временные, поэтому условия в таблице пока не считаем финальными.
- Без фиксированного правила H2, таблица и CTA могут разъехаться между типами страниц.

### Решение

- H2: `Предложения партнеров`;
- таблица предложений выглядит одинаково на всех типах страниц;
- колонки: `Партнер`, `Ставка от`, `Сумма`, `Срок`, `Требования`, `Ссылка`;
- CTA: `Подать заявку`;
- данные предложений дорабатываем позже.
...
- Карточки частично повторяются, хотя блок должен давать уникальность страницы.
...
- H2 гео-интента `Программы кредитования в Екатеринбурге` не подходит для страницы займа.
...
```text
Городской хаб: Что важно знать о займах и кредитах под залог ПТС и авто в {городе}
Гео-интент: Что важно знать про {интент} в {городе}
Гео-модификатор: Что важно знать про {интент} {модификатор} в {городе}
```

Логика 3 абзацев:

```text
Абзац 1 - что это за страница / какой спрос закрывает.
Абзац 2 - как Bablocar помогает в этом сценарии.
Абзац 3 - что важно знать перед заявкой.
```
...
- Есть риск добавить в ответы механику и обещания, которых нет в продуктовой базе.
...
- Кнопка `Рассчитать займ` на городском хабе и гео-интенте не совпадает с базовым CTA.
...
- Текст и действие финального CTA меняются между страницами.
